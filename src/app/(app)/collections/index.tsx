...
export default function AllCollections() {
  ...
  const [selectedImageFromGallery, setSelectedImageFromGallery] = useState<string | null>(null);
  ...
  const HandleConfirmDelete = async () => {
    ...
    try {
      ...
      if (collections) {
        setCollections(collections.filter(c => c._id !== collectionIdToDelete));
      }
  ...
  const HandleUpdateCollection = async () => {
    ...
    const uploadImageIfNeeded = async (): Promise<string> => {
      ...
      return selectedImageFromGallery || selectedImage || collectionToEdit.image || '';
    };
      ...
      >
        <View className="flex-1 bg-black/50 justify-center items-center p-4">
          <View className="bg-white rounded-lg p-6 w-full max-w-md">
            <Text className="text-xl font-bold mb-4" style={{ color: colors.error[500] }}>
              {t('Confirm Deletion')}
            </Text>
            <Text className="text-base mb-6 text-gray-700">
              {t('You are sure you want to delete? All data will be deleted, all cards and all study progress will be lost...')}
            </Text>
